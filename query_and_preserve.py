...
def query_and_preserve(headers1):
    """
    1. 查票（随机高铁或普通）
    2. 查保险、Food、Contacts
    ...
    if high_speed:
        print("high speed quering....")
        start = "Shang Hai"
        end = "Su Zhou"
        high_speed_place_pair = (start, end)
        trip_ids = _query_high_speed_ticket(place_pair=high_speed_place_pair, headers=headers1, time=date)
        PRESERVE_URL = f"{base_address}/api/v1/preserveservice/preserve"
    else:
        print("normal quering....")
        start = "Shang Hai"
        end = "Nan Jing"
        other_place_pair = (start, end)
        trip_ids = _query_normal_ticket(place_pair=other_place_pair, headers=headers1, time=date)
        PRESERVE_URL = f"{base_address}/api/v1/preserveotherservice/preserveOther"

    _ = _query_assurances(headers=headers1)
    food_result = _query_food(headers=headers1)
    contacts_result = _query_contacts(headers=headers1)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        try:
            query_and_preserve(headers1=headers1)
            print("*****************************INDEX:" + str(i))
