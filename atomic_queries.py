...
def _login(username="fdse_microservice", password="111111"):
    ...
    cookies = {
        'JSESSIONID': 'C2FBC9F942C500363720049872A91AC3',
        'YsbCaptcha': '396BC310EE694084B10A12A5DDBFBD24',
    }
...
def _query_advanced_ticket(place_pair: tuple = ("Nan Jing", "Shang Hai"), headers: dict = {}, time: str = "2023-04-07",
...
def _query_food(place_pair: tuple = ("Shang Hai", "Su Zhou"), train_num: str = "D1345", headers: dict = {}):
    url = f"{base_address}/api/v1/foodservice/foods/2023-04-14/{place_pair[0]}/{place_pair[1]}/{train_num}"
...
def _query_orders(headers: dict = {}, types: tuple = tuple([0]), query_other: bool = False) -> List[tuple]:
    ...
    if response.status_code != 200 or response.json().get("data") is None:
        logger.warning(f"query orders failed, response data is {response.status_code}")
        return None
